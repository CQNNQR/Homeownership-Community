...
### CORS / Edge Functions / MFA bypass (June 8, 2026)

Follow-up pass on top of the May 31 remediation: the live admin SPA at `https://www.crossgage.com` was throwing CORS errors on every Supabase Edge Function call, and the build was also warning that MFA enforcement was disabled in production.

#### Symptom

Captured from `logs.txt` on the deployed build (`index-DTTBRvNu.js`):

- 3 `Access to fetch at 'https://uvnmbplvpksanlkntydo.supabase.co/functions/v1/<fn>' from origin 'https://www.crossgage.com' has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.` errors, one each for:
  - `functions/v1/admin-payments` — `POST ... net::ERR_FAILED 403 (Forbidden)`
  - `functions/v1/admin-campaigns` — `POST ... net::ERR_FAILED 403 (Forbidden)`, followed by `[CampaignDetail] Failed to load campaign: TypeError: Failed to fetch`
  - `functions/v1/host-venues` — `GET ... net::ERR_FAILED 403 (Forbidden)`
- 1 console warning: `[AAL2] VITE_E2E_BYPASS_MFA=true — MFA enforcement is DISABLED. This is a test-only escape hatch and must never be set in production.`

Result: the admin `payments`, `campaigns`, and host `venues` screens could not load or mutate any data through the Edge Functions.

#### Root cause

1. The three Edge Functions (`admin-payments`, `admin-campaigns`, `host-venues`) were deployed without any CORS handling. They only attached the headers on the happy path inside `serve()` and never answered `OPTIONS` preflight, so the browser blocked every cross-origin call from `https://www.crossgage.com` with `403 (Forbidden)` and `Failed to fetch`.
2. `VITE_E2E_BYPASS_MFA=true` had been promoted into the Vercel production environment. That variable is a test-only escape hatch (used by the E2E suite to skip MFA); leaving it set in production silently disables MFA enforcement and prints the `[AAL2]` warning on every page load.

#### Fix

- Created `supabase/functions/_shared/cors.ts` with a single source of truth for CORS: an `ALLOWED_ORIGINS` allow-list (`https://www.crossgage.com`, `https://crossgage.com`, and the common local-dev hosts), plus helpers `corsHeaders`, `handlePreflight`, `withCors`, and `jsonResponse`. Every Edge Function now uses these so success, error, and preflight paths all return the same headers.
- Wired the three functions to use the shared helper:
  - `supabase/functions/admin-payments/index.ts`
  - `supabase/functions/admin-campaigns/index.ts`
  - `supabase/functions/host-venues/index.ts`

  Each one now calls `handlePreflight(req)` at the top, wraps every response with `withCors(req, ...)` (or uses `jsonResponse(req, ...)`), and is robust to non-`OPTIONS` early returns.
- Deployed all three with `supabase functions deploy admin-payments admin-campaigns host-venues` (and the shared file is bundled into each deploy).
- Removed `VITE_E2E_BYPASS_MFA` from the Vercel production environment variables.
- Documented it as **testing-only** in `.env.example` under a clearly labeled `# --- Testing-only (NEVER set in production) ---` block, with the line `VITE_E2E_BYPASS_MFA=true` commented out and an explicit "NEVER set in production" warning.

#### Files changed

- `supabase/functions/_shared/cors.ts` — new, shared CORS helper (allow-list, `corsHeaders`, `handlePreflight`, `withCors`, `jsonResponse`).
- `supabase/functions/admin-payments/index.ts` — uses `handlePreflight` + `withCors` / `jsonResponse` on every code path.
- `supabase/functions/admin-campaigns/index.ts` — same.
- `supabase/functions/host-venues/index.ts` — same.
- `.env.example` — adds the "Testing-only (NEVER set in production)" block documenting `VITE_E2E_BYPASS_MFA` as test-only.
- `roadmap.md` — this section.

(Out-of-repo change, recorded for completeness: `VITE_E2E_BYPASS_MFA` was deleted from the Vercel project's production env vars. No source file held the literal value.)

#### Verification

Live `OPTIONS` preflight against each of the three functions now returns the correct CORS headers. Sample (trimmed):

```
$ curl -i -X OPTIONS \
    -H 'Origin: https://www.crossgage.com' \
    -H 'Access-Control-Request-Method: POST' \
    https://uvnmbplvpksanlkntydo.supabase.co/functions/v1/admin-payments

HTTP/2 204
access-control-allow-origin: https://www.crossgage.com
access-control-allow-methods: GET, POST, PUT, PATCH, DELETE, OPTIONS
access-control-allow-headers: authorization, x-client-info, apikey, content-type, x-supabase-auth, x-requested-with
access-control-max-age: 86400
access-control-allow-credentials: true
vary: Origin
```

The same `204 + Access-Control-Allow-Origin: https://www.crossgage.com` response is returned by `admin-campaigns` and `host-venues`. Subsequent `POST` / `GET` calls from `https://www.crossgage.com` no longer hit the CORS preflight blocker; the `[AAL2] VITE_E2E_BYPASS_MFA` console warning is gone from a fresh production load.
